...
本技能定義當本專案任何 Feature / Fix 分支開發結束、準備收尾或準備進行 Merge 時，必須按順序執行的**兩階段硬性狀態閘門 (Two-Phase Gated Workflow)** 與檢核清單。
...
> 必須先進入 **Phase 1 (收斂驗證閘門)**。若本次分支涉及任何 `docs/todos/`、開發 Spec 或核心契約變更，AI **必須停在 Phase 1 向使用者提問候選收斂清單，等待使用者確認後完成契約升格與過期 Spec 清理**。
> 未完成 Phase 1 文件收斂之前，輸出任何 `git merge` 指令均視為嚴重流程違規（Process Regression）！

---

## 🚦 兩階段硬性狀態閘門 (Two-Phase Gated Workflow)
...
使用者輸入：「跑merge / 收尾 / 準備merge」
      ...
      ▼
┌─────────────────────────────────────────────────────────┐
│ 【Phase 1: 收尾驗證與文件收斂閘門 (Archival & Docs Gate)】 │
├─────────────────────────────────────────────────────────┤
│ 1. 執行最小聚焦單元測試，確保改動邏輯 100% 通過。        │
│ 2. 撰寫 PARS 框架開發故事 (docs/storys/)。              │
│ 3. 檢查 git diff main..HEAD 是否有 docs/todos/ 或 Spec？│
│    ├─ 有 ──> 🛑 強制阻斷！向使用者列出候選清單提問。      │
│    │         等待使用者確認後，執行契約升格與 Spec 清理。  │
│    └─ 無（或已確認收斂完成） ──> 解鎖 Phase 2             │
└────────────────────────────┬────────────────────────────┘
                             │ (Phase 1 完成解鎖)
                             ▼
┌─────────────────────────────────────────────────────────┐
│ 【Phase 2: 分支審計與合併指令交付 (Merge Command Delivery)】│
├─────────────────────────────────────────────────────────┤
│ 4. 執行 git log / git diff 彙整異動統計與模組細節。     │
│ 5. 生成 Windows/Linux 相容之標準 --no-ff 合併指令。    │
│ 6. 提醒使用者手動執行全套單元測試。                     │
└─────────────────────────────────────────────────────────┘
```
...
### Phase 1：收尾驗證與文件契約收斂 (Archival & Docs Gate)
...
1. **前置交互守則（強制提問）**：
   ...
   - **🛑 停在此處等待回覆**：在使用者確認清單前，**絕對不得輸出 Phase 2 的 git merge 指令**！
...
### Phase 2：分支審計與合併指令交付 (Merge Command Delivery)
*（僅當 Phase 1 文件收斂已完成或確認無需收斂時，方可進入本階段）*
...
- ⚠️ **嚴格順序性**：Phase 1 未確認完成前，絕不可輸出 Phase 2 的 merge 指令。
