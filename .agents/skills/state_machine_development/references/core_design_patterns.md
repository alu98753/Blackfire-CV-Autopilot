...
* **規範**：
  ...
  - ✅ **必須**在全入口 (CLI 無 I/O 旗標預設 Backend Production、`--foreground`、`--subflow`, `main.py` 與 `GameStateMachine`) 設定 config 時，一律呼叫 `normalize_config(config)`，確保 `disassemble_colors` (`["gray_or_empty", "green", "blue"]`) 與 `keep_colors` 在任何啟動與切換時都 100% 精準。
