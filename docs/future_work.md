...
```
- **[已完成] 同樣是假設我在戰鬥途中因為使用者或甚麼原因導致意外退出 他會無法從戰鬥模式的檢測跳出導致卡死 應該在battle下 偵測有沒有 幾個戰鬥途中會有的圖片(我要給你) 如果沒有 那就要思考一下 應該跑到了大廳 or door 或意外點到了東西(要點quit continue等 其中一個或多個直到退出) 
