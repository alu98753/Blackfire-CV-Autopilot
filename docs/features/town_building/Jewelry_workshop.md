...
使用者可在 [config.py](../../../config.py#L255) 中的 `goods_settings` 字典內，按顏色品質區分並**個別管理每一個商品是否出售 (`True` / `False`)**：
> ⚠️ **注意事項**：新增任何商品截圖至 `goods/` 資料夾時，必須同步在 [config.py](../../../config.py#L255) 的 `goods_settings` 白名單設定為 `True`，系統才會發起比對與出售！
