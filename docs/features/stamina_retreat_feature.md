...
> 本規則的長期義務是「退避期間不得啟動常規關卡，且合格地下城可在不重設退避生命週期的前提下暫時執行」。設定欄位、旗標名稱、時間單位與路由組裝方式可隨重構調整；驗證以 `tests/test_behavior_stamina_retreat.py`、`tests/test_daily_pipeline_stamina_retreat.py` 與 `tests/test_stamina_retreat_routing.py` 為準。術語判讀見 [Canonical Invariant Registry](../architecture/canonical_invariant_registry.md)。
