...
- [ ] Config ownership foundation: [`activity-execution-config-ssot`](todos/activity-execution-config-ssot.md) — 建立每個 Activity 的 execution-config SSOT；讓 Daily/Mix 等上層 policy 只選 Activity/target，不再各自維護 Stage/Dungeon/Domain 的 navigation/templates/buttons 等 execution 欄位。此項先於 Activity orchestration consolidation。
- [ ] Activity orchestration consolidation: [`activity-plan-mode-consolidation`](todos/activity-plan-mode-consolidation.md) — 在 canonical Activity config ownership 成立後，統一 `daily` / `custom` ActivityPlan、Scheduler / Intent lifecycle 與直接 `--activity` 開發入口，逐步淘汰 `type="mix"` / `--subflow` 作為長期 orchestration 概念；完成後再做 bounded Navigation legacy retirement。
- [ ] Navigation cleanup closeout: [`navigation-legacy-retirement`](todos/navigation-legacy-retirement.md) — 待 Activity execution config SSOT 與 ActivityPlan/mode consolidation 完成後，證明剩餘 compatibility callers unreachable，再刪除 Stage/Domain/fixed-Dungeon/Lord/Demon-Lord 與 cross-mode routing 的 legacy navigation ownership；Greedy Dungeon target selection 與各 mode downstream business logic 明確非刪除目標。
