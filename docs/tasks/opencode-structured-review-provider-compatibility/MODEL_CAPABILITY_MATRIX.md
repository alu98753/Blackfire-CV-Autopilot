...
- **Transport / Structured-output Capability**: **PROVEN**. Candidate C3 successfully completes JSON-Schema structured transport through official `@opencode-ai/sdk/v2` without triggering the Phase 1 OpenAPI prompt crash.
- **Formal Route Qualification**: **NOT PROVEN**. The route cannot be classified as `PASS_PROVEN` under the Final SPEC because finalization was forced by step budget exhaustion (`finish: tool-calls`), and `client.session.messages` decoder failure prevents trustworthy third-party audit of whole-session tool activity.
