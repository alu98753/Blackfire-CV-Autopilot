...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `C3` | `1.18.31` | `@opencode-ai/sdk@1.18.31` (SDK v2) | `opencode/big-pickle` | `regression-reviewer` | diagnostic (480s) | PROVEN (HTTP 200, `info.structured`) | NOT PROVEN | Step-budget exhausted (`tool-calls`); `session.messages` decoder failure blocks independent lifecycle audit |
| `C3` | `1.18.31` | `@opencode-ai/sdk@1.18.31` (SDK v2) | `opencode/mimo-v2.5-free` | `regression-reviewer` | diagnostic (480s) | PROVEN (HTTP 200) | NOT PROVEN | Voluntarily stopped (`finish: stop`), but omitted structured output (`info.structured = undefined`); classifies as `FAIL_STRUCTURED_OUTPUT` |
...
- **Formal Route Qualification**: **NOT PROVEN**. Neither candidate achieves `PASS_PROVEN` under the Final SPEC: Big Pickle exhausted step budget (`finish: tool-calls`), and MiMo failed to emit structured output on the full production review prompt (`finish: stop` with missing `info.structured`). Additionally, `client.session.messages` decoder failure prevents trustworthy third-party audit of whole-session tool activity.
