...
- Model fallback in `ai_gate.ps1` manages ONLY automated independent OpenCode reviewer candidates. Gate review routing is capability-constrained: every configured candidate must support JSON-Schema structured output and have sufficient finalization reliability after read/search tool calls; a candidate lacking either prerequisite is not a valid Gate reviewer route.
