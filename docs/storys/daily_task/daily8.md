...
5. 打boss (首領領主討伐)
   - **規則**：每個 Boss 獨立計算（每天上限 5 次，每次戰鬥後冷卻 2 小時）。例如：「育母蜘蛛」與「古代惡靈」各自有獨立的 5 次與 2 小時 CD。
   - **狀態持久化**：於 `user_data/daily_status.json` 中紀錄每個 Boss 的獨立 `today_count` 與 `last_fight_timestamp`。
   - **每日 08:30 自動重置**：當時間跨過 08:30 時自動將所有 Boss 的今日計數清零並重置任務狀態。
