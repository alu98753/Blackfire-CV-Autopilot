...
專案中處理例外與卡死的模組位於 [states/exceptions/](../../states/exceptions) 目錄，主要採用 **雙層救援機制 + 重開遊戲兜底** 的架構。
...
#### 1. 看門狗監控器：[ExceptionWatchdog](../../states/exceptions/watchdog.py)
* **狀態時間判斷**：
...
1. **降低第二階段重開的門檻**：
   在 [handler.py](../../states/exceptions/handler.py) 中，當 `UnexpectedPopupRecoveryHandler` 重試 5 次點擊仍無法消除畫面阻礙（且畫面未發生任何變化）時，可直接呼叫 `GameRelaunchSubflow` 重開，而不需要退回原狀態再等第二次 90 秒。
2. **避免無效的狀態切換**：
