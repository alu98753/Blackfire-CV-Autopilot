...
def _run_live_deletion_verification(
    notifier: DiscordWebhookAdapter,
    message_id: str,
...
def _verify_reconcile_test_invariants(
    ...
    remaining_expired = [
        m for m in coordinator.history.get("dispatched_messages", [])
        if str(m.get("date", "")) < today_tag
    ]
    assert_dispatched_empty = (len(remaining_expired) == 0)
    assert_latched = (coordinator.history.get("last_reconciled_date") == today_tag)
...
def _run_live_reconcile_verification(
    ...
    from states.daily_pipeline_notifier import DailyPipelineNotifier, resolve_business_dt
    ...
    with tempfile.TemporaryDirectory(prefix="blackfire_live_reconcile_") as temp_dir:
        ...
        coord.history["dispatched_messages"].append({
            "id": msg_id,
            "date": yesterday_tag,
