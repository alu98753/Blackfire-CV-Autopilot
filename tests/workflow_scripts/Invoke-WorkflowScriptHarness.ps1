...
try {
    Run-Case 'Reviewer budgets and bounded finalization contract' {
        ...
        foreach ($text in @($specText, $regressionText)) {
            ...
            Assert-True ($text -match 'Batch independent reads/searches in the same tool-bearing turn when supported') 'batched exploration guidance missing'
            Assert-True ($text -match 'If evidence is incomplete at the exploration cutoff, decide from the grounded evidence already collected') 'incomplete evidence decision rule missing'
            Assert-True ($text -match 'a concrete contract violation is BLOCK; otherwise record the uncertainty or evidence gap as advisory and finalize') 'uncertainty finalization rule missing'
        }
        Assert-True ($specText -match 'By the end of step/turn 6, stop all tool use') 'spec exploration cutoff missing'
        Assert-True ($specText -match 'Steps 7-8 are reserved for synthesis, structured finalization, and emitting the canonical outcome only') 'spec final two turns reservation missing'
        Assert-True ($regressionText -match 'By the end of turn 8, stop all tool use') 'regression exploration cutoff missing'
        Assert-True ($regressionText -match 'Turns 9-10 are reserved for synthesis, structured finalization, and emitting the canonical outcome only') 'regression final two turns reservation missing'
    ...
    Run-Case 'Structured reviewer adapter uses isolated ephemeral database' {
        ...
        Assert-True ($adapterText -match 'SAFE_INFO_FIELDS') 'adapter missing-output diagnostics must use an info allowlist'
        Assert-True ($adapterText -match 'structured_output; info=') 'adapter must include bounded info metadata when structured output is absent'
        Assert-True ($adapterText -notmatch 'result\.data\.parts|info\.parts') 'adapter must not use response parts as machine authority'
