...
### 🧮 2. 2-10「Boss 果實 + 伴隨小怪岩石碎片」雙通道雙產物綜合進度模型

在 2-10 戰鬥中，**Boss 傀儡**與**伴隨小怪（蜥蜴/蜘蛛）** 同時提供可用於合成精煉鋼錠的素材：

* **等價素材需求**：
  $$\mathbf{1 \times 精煉鋼錠} = \mathbf{8 \times 堅石果實} \quad (\mathbf{OR}) \quad \mathbf{16 \times 粗鐵錠} \ (=\mathbf{32 \times 硬化岩石碎片})$$
* **各產物單場合成進度貢獻**：
  1. 🍈 **Boss 堅石果實**：需要 8 顆 $\rightarrow$ 掉率 $49.42\%$，單場進度貢獻 = $\frac{1}{8} \times 49.42\% = \mathbf{6.178\%}$
  2. 🧱 **小怪硬化岩石碎片**：需要 32 顆 $\rightarrow$ 掉率 $33.33\%$ ($1/3$)，單場進度貢獻 = $\frac{1}{32} \times 33.33\% = \mathbf{1.042\%}$

#### 🎯 單場 2-10 精煉鋼錠綜合期望合成進度：
$$\mathbf{\text{單場期望進度}} = \left(\frac{1}{8} \times 49.42\%\right) + \left(\frac{1}{32} \times 33.33\%\right) = 0.061775 + 0.010417 = \mathbf{0.0722 \quad (7.22\%)}$$

#### 📦 合成 1 個精煉鋼錠的期望需刷場次：
$$\mathbf{\text{期望總場次}} = \frac{1}{0.072192} \approx \mathbf{13.85 \sim 14 \text{ 場}}$$
* **體力總消耗**：$14 \text{ 場} \times 2 \text{ 體力} = \mathbf{28 \text{ 體力}}$！

---

### 🧮 3. 第 5 章沼澤 鐵礦石 (`iron_ore`) 單場掉落率推導（實機修正版）
