...
| **4 星 (黃色傳說)** | `rarity: 4.0` | **`5.0%`** | 傳說裝備、熔岩礦石、高級精華素材 |
| **5 星 (橙色神話)** | `rarity: 5.0` | **`2.5%`** | 蜘蛛皇家核心、巨龍核心、神話遺物 |
| **6 星 (紅色遠古/創世)** | `rarity: 6.0` | **`1.25%`** | 罪業核心、創世神裝圖紙、六階精華 |
...
> 🏆 **實測範例 A (2-10 Boss 1星綠色素材·堅石果實)**：
> $$P(\text{果實}) = 40.0\% + \Big(25.92\% \times \frac{40}{110}\Big) = 40.0\% + 9.42\% = \mathbf{49.42\% \quad (\approx 50\%)}$$
> 打 27 場實測獲得 15 個果實（$55.5\%$），與理論模型完美契合！

---

## 🔬 五之二、高階稀有裝備/飾品（4星橘色以上）實戰掉落校準與多帳號大數據實測

> [!IMPORTANT]
> **多帳號長期實測實證 (沙漠遺跡 10 關 4星骷髏護符 `accessory_skeleton`)**：
> 經 2～3 個帳號各累計 100～300 場長期掛機數據統計，**4星橘色裝備/飾品打 100 場未掉落為常態，平均需 200～300 場才會產出 1 件**（實質綜合產出率約 **`0.33% ~ 0.50%`**）。

### 🔍 實戰產出率（0.3%~0.5%）與純星級數值（5.0%）的 3 大底層差異成因：

1. **裝備/飾品大類別檢定門檻 (Category Slot Filter)**：
   * 擊殺 Boss 時並非每次都直接結算裝備飾品，大部分掉落名額優先分配給基礎素材、魔物血液與貨幣。裝備/飾品類別的觸發率通常約為 **$10\%$**。
   * 實質掉落率 $= 10\% (\text{類別門檻}) \times 5.0\% (\text{4星稀有度}) = \mathbf{0.50\%}$（即平均 **200 場 / 件**）。
2. **全場小怪素材池的權重競爭與稀釋 (Pool Dilution)**：
   * 沙漠 10 關全場怪（骷髏戰士、骷髏法師、骷髏王）總權重高達 **535 點**，4星護符（權重 5 點）在總池中僅佔 **$0.93\%$**。在單場有限戰利品名額中，極易被同場小怪龐大的低階素材擠佔。
3. **無累積大保底下的乾旱期波動 (Variance in Independent Trials)**：
   * 在真實機率 $P \approx 0.5\%$ 下，單純連續 100 場未命中的機率為 $(1 - 0.005)^{100} \approx \mathbf{60.5\%}$。這表示**打 100 場有超過 6 成機率空手而歸**，必須累積滿 200～300 場才跨過數學期望值門檻。

### 📊 低星素材 vs 高星裝備實戰掛機規劃對照表：

| 物品類型 | 代表項目 | 稀有度星級 | 底層 `rarity.chances` | 實測單件期望場次 | 建議掛機週期 |
| :--- | :--- | :---: | :---: | :---: | :---: |
| **低階材料** | 堅石果實、蜥蜴獸石、粗鐵 | 1星 ~ 2星 | 40% ~ 20% | **2 ~ 5 場 / 件** | 10 ~ 30 場 |
| **中階素材/裝備** | 傀儡飾品、冰霜礦石 | 3星 | 10% | **10 ~ 25 場 / 件** | 50 ~ 100 場 |
| **高階傳說/飾品** | **骷髏護符**、首領專屬裝備 | 4星 | 5% | **200 ~ 300 場 / 件** | **300 ~ 500 場** |
| **神話/上古核心** | 蜘蛛皇家核心、罪業核心 | 5星 ~ 6星 | 2.5% ~ 1.25% | **400 ~ 800 場 / 件** | **500 ~ 1000 場** |
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| 🟡 **4 星 (黃色傳奇)**<br>【傳奇貢品】 | **3 次** | **`5.00%`** | **`6.00%`** (+20%) | **`7.00%`** (+40%) | **`8.00%`** (+60%) | **`+60.0%`**<br>(產出增為 1.6 倍) |
| 🟠 **5 星 (橙色神話)**<br>【神話貢品】 | **2 次** | **`2.50%`** | **`3.50%`** (+40%) | **`4.50%`** (+80%) | 🔒 *已達購買上限* | **`+80.0%`**<br>(產出增為 1.8 倍) |
| 🔴 **6 星 (紅色上古)**<br>【上古貢品】 | **1 次** | **`1.25%`** | **`2.25%`** (+80%) | 🔒 *已達購買上限* | 🔒 *已達購買上限* | **`+80.0%`**<br>(產出增為 1.8 倍) |
