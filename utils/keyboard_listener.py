...
class PauseController:
    """
    熱鍵策略與前景視窗焦點控制器 (PauseController)
    
    支援三大核心熱鍵（雙視窗焦點過濾：遊戲視窗焦點/懸停、或終端機視窗）：
    1. 【Ctrl + Space】：暫停 / 恢復掛機（原地定格，不殺進程）。
    2. 【Ctrl + Q】：手動快速無縫重啟（發送 Exit 42 ➔ Supervisor 0秒以 --resume 重開）。
    3. 【Ctrl + Shift + Q】：手動完全安全退出（發送 Exit 75 ➔ Supervisor 乾淨關閉）。
       
    核心架構：
    - 獨立背景守護執行緒 (Background Daemon Thread，每 10ms 採樣) 確保主執行緒 OpenCV/OCR 阻塞時 100% 毫秒級捕獲。
    - 嚴格雙視窗焦點過濾 (Terminal 或 Game 視窗聚焦/懸停時生效，第三方視窗 100% 忽略)。
    """
    ...
    def check_toggle_triggered(self) -> bool:
        ...
        :return: True 代表成功觸發切換；False 代表無事件
        """
    ...
    def _poll_once(self, now=None):
        """
        單次按鍵檢測：僅包含 Ctrl+Space (暫停)、Ctrl+Q (快速重啟)、Ctrl+Shift+Q (完全退出)。
        """
        if now is None:
            now = time.time()
