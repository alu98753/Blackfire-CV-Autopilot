...
- **[已擱置] 為了讓稀有戰利品不被誤分解(即使現在可以判斷顏色了),我希望在戰鬥結束畫面(也就是continue處) 判斷到"獲得戰利品" 跟continue同時存在時 去紀錄有哪些要鎖定 如果有 就接著去背包鎖定(如果已經鎖定(只有解鎖的字樣沒有鎖定的字樣) 就退出 (不要再繼續按了))) 如果沒有就再戰鬥 retry