...
- **[待辦] 在地下城模式下的bug : 每層探索結束後 會是  
```
1. 🔍 檢查自動戰鬥按鈕... 最大相似度: 0.4721 (閥值: 0.7)

2. 🏆 戰鬥結束！點擊相似度最高的地下城結算按鈕 [common/continue_gray.png]，信心度: 0.8952
3. [後台點擊] 物理相對: (960, 674) -> 邏輯相對: (951, 636) [DPI 縮放: 1.25]

但是此時應該有第二個continue , 但她卻會先寫

⏳ 下樓冷卻結束，已進入地下城新樓層，重設探索記憶。
2026-07-09 10:54:11,126 [INFO] ⌛ 地下城探索中，正在等待下一層載入或新的隨機事件按鈕出現...

然後過了一些延遲, 才又再次另一個 continue_gray, 但這邊沒有print 因此不知道寫的邏輯是什麼(我覺得應該是有確認就點確認的fallback邏輯? 這個是可以保留的)

總之 在⏳ 下樓冷卻結束，已進入地下城新樓層，重設探索記憶。 前 少了一個應該要按下的continue
```
