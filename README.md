...
7. **滑鼠手動介入偵測與鎖定狀態暫停 (Anti-Interference Manual Pause)**：
 ...
 9. **背包滿自適應裝備分選與低稀有度銷毀 (Backpack Full Smart Sorting & Discard)**：
    * 當畫面上彈出「無法容納的物品 (背包已滿)」彈窗 (`templates/backpack_full.png`)，或啟動腳本直接處於此畫面時，腳本會立即轉移至專屬的分選狀態。
    * **HSV 色彩空間內圈環狀取樣法**：精確分析左側溢出格與右側背包格（取樣 offset 10~20 區間，避開外層黃金格框）判定裝備稀有度。
    * **銷毀與收納循環**：若左側有貴重物品（藍、紫、黃、橘、紅邊框），腳本會點選右側的低稀有度物品（綠色或灰色） ➔ 點擊銷毀按鈕 (`templates/destroy.png`) ➔ 確認確認，隨後將左側貴重物品點選收入背包中。
    * **背包滾動搜尋與安全回退**：若當前頁面無低稀有度物品，會自動向下滾動背包（最多3次）尋找。若最終仍無低稀有度物品可銷毀，則記錄警告並點選退出關閉，絕不卡死。
 10. **模組化狀態機設計**：
